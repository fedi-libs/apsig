...
async def send(request: Request):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        target = URL(url)
        signer = RFC9421Signer(private_key=ed_privatekey, key_id="https://apsig.amase.cc/actor#ed25519-key", method="post", path="/inbox", host=target.host + ":" + str(target.port) if target.port is not None else "", headers={"Content-Type": "application/activity+json"})
