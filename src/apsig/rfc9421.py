...
class RFC9421Signer:
    ...
    def __build_signature_base(self, special_keys: dict, headers: dict) -> bytes:
        headers_new = []
        headers = headers.copy()
        for h in self.sign_headers:
            if h in ["@method", "@path", "@authority"]:
    ...
    def sign(self, method: str, path: str, host: str, headers: dict, body: bytes | dict=b""):
        if isinstance(body, dict):
            body = json.dumps(body).encode("utf-8")

        if not headers.get("Date"):
            headers["Date"] = email.utils.formatdate(usegmt=True)
        if headers.get("content-length") is None or headers.get("Content-Length") is None:
        ...
        with open("test.txt", "w") as f:
            f.write(base)
        print("\n\n")
        signed = self.private_key.sign(base.encode("utf-8"))
